...
export default function Contact() {
  ...
  return (
    <div>
      {submitted ? (
        <div>
          <h1 className='text-white'>Thank you for submitting the form.</h1>
        </div>
      ) : (
        <form className='h-screen text-white contact-bg' ref={form} onSubmit={sendEmail}>
          ...
          <button type="submit" className='contact-btn shadow-lg' onClick={setSubmitted({submitted: true})}>SUBMIT</button>
        </form>
